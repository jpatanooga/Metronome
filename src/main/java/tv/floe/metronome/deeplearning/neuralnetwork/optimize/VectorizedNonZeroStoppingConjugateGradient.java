...
import tv.floe.metronome.math.OptimizerMatrix;
...
//				listener.epochDone(iterationCount);
			...
			gg = MatrixUtils.sum( MatrixUtils.pow(g, 2) );
			...
			h = xi.clone().plus( h.times(gam) );
			
			
			//assert (!MatrixUtil.isNaN(h));
			assert( !MatrixUtils.isNaN( h ) );
